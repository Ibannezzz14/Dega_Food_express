...
type AdminAvatarRouteContext = {
  params: Promise<{ id: string }>;
};

export async function GET(
  request: NextRequest,
  { params }: AdminAvatarRouteContext,
) {
  if (
    !isStatsAuthorizationValid(request.headers.get("authorization"))
  ) {
  ...
  try {
    ...
    logServerError("review_avatar_database_failed", error, {
      ...
      route: "/statistiques/avis/avatar/[id]",
      operation: "read_admin_avatar",
    });
    return new Response(null, { status: 404 });
  }
}
